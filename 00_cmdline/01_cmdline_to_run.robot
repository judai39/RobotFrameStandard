...
CASE01
    ...
    Log To Console    在user.py库中调用name值为:${User.name}
    Log To Console    在user.py库中调用name值为:${User.get_user_name()}
...
# 如果要传入的时library，需要使用variablefile参数 robot --variablefile ../.library/User.py .\01_cmdline_to_run.robot
